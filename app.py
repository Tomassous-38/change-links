...
async def update_links(markdown_text, domain, target_language, status_placeholder):
    all_links = get_all_links_from_markdown(markdown_text)
    domain_links = [link for link in all_links if is_same_domain(link, domain)]
    
    status_placeholder.text(f"Processing {len(domain_links)} links...")
...
if st.button('Update Links'):
    if markdown_text and domain_input and target_language:
        ...
        status_placeholder.text("Starting link update process...")
        
        try:
            start_time = time.time()
            updated_markdown, alternate_urls = asyncio.run(update_links(markdown_text, domain, target_language, status_placeholder))
            end_time = time.time()

            status_placeholder.success(f"Links updated in {end_time - start_time:.2f} seconds!")
